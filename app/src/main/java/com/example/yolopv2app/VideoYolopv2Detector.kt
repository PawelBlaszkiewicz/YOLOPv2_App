...
internal class VideoYolopv2Detector(
) {
...
//####################################################################### OBJECT DETECTION CODE
            ...
            var detectionOut = processDetection(
                mutableBitmap,
                pred,
                confThreshold,
                ratiow,
...
//####################################################################### LANE_LINE CODE
            ...
            val upperHorizon = Pair(imgMiddle.first.toInt(), topPointDriveableArea.second) //(0.75 * height).toInt()
...
//################################# TOP LINES ##############################
            ...
            if (leftLine.isNotEmpty() and rightLine.isNotEmpty()){ // drawing top of lines connected
                ...
                if (correctedRightPoint != null && correctedLeftPoint != null) {
            ...
            val result = VideoResult(det_out, flag, cFlag)

            return result
            }
        }
    }
    fun processWarning(bitmap: Bitmap, leftLane: List<Pair<Int, Int>>, rightLane: List<Pair<Int, Int>>, carMiddle:  Pair<Float, Float>, safetyFlag: Int,
    ...
    fun drawFPS(canvas: Canvas, height: Int, width: Int, text: String, textColor: Int){
        ...
        var textSiz = (height/25).toFloat()
        while (textWidth+25 > width){
    ...
    fun processDetection(bitmap: Bitmap, pred: ArrayList<FloatArray>, confThreshold: Float,
        ...
        for (i in pred.indices) {
            ...
            val maxInArray = pred[i].sliceArray(5 until pred[i].size).max()
